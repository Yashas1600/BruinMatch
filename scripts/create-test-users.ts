...
async function createTestUsers() {
  ...
  if (!supabaseServiceKey) {
    ...
    console.log('2. Run: SUPABASE_SERVICE_ROLE_KEY=your_key npx tsx scripts/create-test-users.ts')
    process.exit(1)
  }
  ...
  console.log('Creating test users for "spongebob" dating pool...\n')
  ...
  for (const user of testUsers) {
    try {
      ...
      if (!authData.user) {
        console.error(`❌ No user data returned for ${user.name}`)
        continue
      }
  ...
  console.log('\nYou can now create your account with the code "spongebob" to see these users.')
}
